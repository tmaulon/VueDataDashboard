<template>
  <section id="pageSplash">
    <Logo class="VddApp_Logo" :logo="logo" :src="logo.src" :alt="logo.alt"/>
    <h3 class="VddApp_SplashScreen_Title">Incident Database</h3>
    <div>
      <button @click="logIn()" id="sign-in-button" class="VddApp_Header_UserLogBtn">
        <i class="fas fa-user-circle"></i> Sign in with Google
      </button>
    </div>
  </section>
</template>
